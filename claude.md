# Handy Utils — Claude Code Instructions
...
When implementing any new feature or fix — as part of the same commit, not after:
1. Bump the tool's `version` string and `updatedAt` in `app/page.tsx`
2. Add a new release entry at the top of that tool's `releases[]` array in `app/changelog/release-notes.ts`
3. If the tool has no entry in `release-notes.ts` yet, add one

Version scheme: `MAJOR.MINOR` — increment MINOR for any user-visible change.
